...
public:
    ...
    virtual const char* open(void * di8, void * hwnd, guid_container * guids)
    {
        ...
        if (lpdMouse->SetCooperativeLevel(hWnd, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE) != DI_OK)
    ...
    virtual void poll_mouse()
    {
        ...
        if (FAILED(lpdMouse->GetDeviceState(sizeof(DIMOUSESTATE), (LPVOID)&mouse_state)))
        {
            lpdMouse->Acquire();
            if (FAILED(lpdMouse->GetDeviceState(sizeof(DIMOUSESTATE), (LPVOID)&mouse_state)))
                memset(&mouse_state, 0, sizeof(mouse_state));
